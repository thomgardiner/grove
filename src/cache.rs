...
pub fn acquire(root: &Path, workspace: &str, toolchain: &str) -> Result<Lane> {
    ...
    let lock_file = File::create(lock_path(root, &id)).context("opening lane lock")?;
    lock_file.lock_exclusive().context("locking lane")?;
    ...
    fs::write(dir.join(".grove-meta.json"), serde_json::to_vec(&meta)?)?;
...
/// `target/` is warm and is left untouched.
pub fn seed(lane: &Lane, canonical: &Path) -> Result<bool> {
    ...
    if let Some(meta) = meta {
        fs::write(lane.dir.join(".grove-meta.json"), meta)?;
    }
    Ok(true)
}

pub fn promote(lane: &Lane, canonical: &Path) -> Result<()> {
...
fn remove_lane(root: &Path, dir: &Path, id: &str) {
...
pub fn reclaim_stale(root: &Path) -> Vec<String> {
    ...
    for dir in lanes(root) {
        ...
        if let Some(lock) = try_own(root, &id) {
            remove_lane(root, &dir, &id);
            drop(lock);
            reclaimed.push(id);
        }
    }
...
pub fn enforce_watermark(root: &Path) -> Vec<String> {
    ...
    loop {
        ...
        let Some((_, dir, id)) = candidates
            .into_iter()
            .find(|(_, _, id)| try_own(root, id).is_some())
        else {
            break; // nothing evictable (all in use)
        };
        remove_lane(root, &dir, &id);
        evicted.push(id);
    }
