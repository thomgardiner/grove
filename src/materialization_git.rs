...
impl Failure {
    pub(super) fn classify(code: Option<i32>, detail: &str) -> Self {
...
pub(crate) fn add(input: &Add<'_>) -> Result<(), Failure> {
...
pub(crate) fn sparse(workspace: &Path, cones: &[String]) -> Result<Vec<String>, Failure> {
...
pub(crate) fn full(workspace: &Path) -> Result<(), Failure> {
...
pub(crate) fn cones(workspace: &Path) -> Result<Option<Vec<String>>, Failure> {
...
pub(crate) fn head(workspace: &Path) -> Result<String, Failure> {
    run(
        workspace,
