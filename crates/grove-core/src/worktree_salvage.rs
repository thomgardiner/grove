...
pub fn salvage_work<M>(worktree: &Path, lease: &Lease<M>) -> Result<Option<String>> {
...
pub fn preflight<M>(worktree: &Path, lease: &Lease<M>) -> Result<()> {
    let current_head = text(worktree, &["rev-parse", "--verify", "HEAD"], None)?;
...
fn finish<M>(worktree: &Path, lease: &Lease<M>, archive: &Archive) -> Result<()> {
