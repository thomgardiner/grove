...
impl Snapshot {
    ...
    pub fn index_tree(&self) -> Result<&str> {
        self.index_tree
            .as_deref()
...
pub fn workspace_lock(root: &Path, workspace: &Path) -> Result<File> {
    ...
    let file = File::create(locks.join(format!(
        ...
        repo_slug(&workspace.to_string_lossy())
    )))
...
pub fn persist(root: &Path, repo: &str, snapshot: &Snapshot) -> Result<Ref> {
    ...
    let dir = root.join("snapshots").join(repo_slug(repo));
...
fn manifest_lock(root: &Path, repo: &str) -> Result<File> {
    let locks = root.join("locks");
    fs::create_dir_all(&locks)?;
    let path = locks.join(format!("snapshot-manifest-{}.lock", repo_slug(repo)));
