...
use crate::claim;
use crate::task::{
    self, CommandState, Lifecycle, RecoveryRecord, Task, process_live, starting_pending,
};
...
fn active_reason(task: &Task, lane_busy: bool) -> Option<String> {
    if lane_busy {
...
fn candidate(
    ...
    if let Some(reason) = active_reason(&task, lane_busy) {
...
where
    A: Fn(&Task) -> bool,
    P: Fn(&Task) -> Result<bool>,
    R: Fn(&Task) -> Result<Option<String>>,
{
    ...
    let tasks = task::records_readonly(root, repo)?;
    for task in tasks {
        let Some((task, reason)) = selected(
            candidate(root, repo, &task.id, ttl, false, lane_busy(&task))?,
            &mut report,
        ) else {
            continue;
        };
        let leased = match preflight(&task) {
            Ok(leased) => leased,
            Err(error) => {
                report.skipped.push(SkippedTask {
                    id: task.id,
        ...
        let Some((task, reason)) = selected(
            candidate(root, repo, &task.id, ttl, true, lane_busy(&task))?,
            &mut report,
        ) else {
            continue;
        };
