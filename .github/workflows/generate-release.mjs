...
const unixCapabilityContract = 'const c=JSON.parse(process.argv[1]),v=process.argv[2],s=c.status,i=c.inspection; if(c.grove_version!==v||c.schema_version!==1||s.repository_schema!==1||s.task_status_schema!==4||s.task_record_schema!==6||!c.task.exec_capabilities.includes("edit")||c.task.verification_policy_pinned!==true||c.coordination.mcp_tools!==true||c.coordination.agent_identity_required!==true||c.coordination.git_write_serialization!==true||i.binding_schema!==1||i.execution_schema!==1||i.finish_source_cas!==true||i.process_tree!=="unix_process_group_best_effort"||i.filesystem!=="read_only_permissions_and_digest"||i.output!=="captured_logs_json_report")process.exit(1)';
const windowsCapabilityContract = "if ($capabilities.grove_version -ne $releases[0].app_version -or $capabilities.schema_version -ne 1 -or $capabilities.status.repository_schema -ne 1 -or $capabilities.status.task_status_schema -ne 4 -or $capabilities.status.task_record_schema -ne 6 -or -not ($capabilities.task.exec_capabilities -contains 'edit') -or $capabilities.task.verification_policy_pinned -ne $true -or $capabilities.coordination.mcp_tools -ne $true -or $capabilities.coordination.agent_identity_required -ne $true -or $capabilities.coordination.git_write_serialization -ne $true -or $capabilities.inspection.binding_schema -ne 1 -or $capabilities.inspection.execution_schema -ne 1 -or $capabilities.inspection.finish_source_cas -ne $true -or $capabilities.inspection.process_tree -ne 'windows_job_object' -or $capabilities.inspection.filesystem -ne 'read_only_permissions_and_digest' -or $capabilities.inspection.output -ne 'captured_logs_json_report')";
